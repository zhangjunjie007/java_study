...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-3h0IUYz3-1662817768299)(./pictures/java内存区域/JVM运行时数据区域.png)]

**JDK 1.8** ：

[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-zRegenDg-1662817768300)(./pictures/java内存区域/Java运行时数据区域JDK1.8.png)]
...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-wj6PYEgR-1662817768301)(./pictures/java内存区域/stack.png)]
...
![](https://img-blog.csdnimg.cn/img_convert/798b0171a4bd5750d4964515fa547451.png)
...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-Fg5xGBuw-1662817768301)(./pictures/java内存区域/《深入理解虚拟机》第三版的第2章-虚拟机栈.png)]
...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-U31LedJS-1662817768302)(./pictures/java内存区域/hotspot-heap-structure.png)]
...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-iQuZvik4-1662817768302)(./pictures/java内存区域/method-area-implementation.png)]
...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-kon1EhWw-1662817768302)(./pictures/java内存区域/method-area-jdk1.6.png)]

[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-D5EgygSo-1662817768302)(./pictures/java内存区域/method-area-jdk1.7.png)]

[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-HxJXokHV-1662817768303)(./pictures/java内存区域/method-area-jdk1.8.png)]
...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-WnVCPvPy-1662817768303)(./pictures/java内存区域/对象的访问定位-使用句柄.png)]

#### 直接指针

如果使用直接指针访问，那么 Java 堆对象的布局中就必须考虑如何放置访问类型数据的相关信息，而 reference 中存储的直接就是对象的地址。

[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-6Z7xSGWU-1662817768303)(./pictures/java内存区域/对象的访问定位-直接指针.png)]
